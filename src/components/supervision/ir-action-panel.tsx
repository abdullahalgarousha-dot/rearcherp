...
import { updateInspectionRequestStatus, resubmitInspectionRequest, approveIR } from "@/app/admin/supervision/actions"
import { toast } from "sonner"
import { useRouter } from "next/navigation"
import { CheckCircle2, XCircle, RefreshCw, Upload, Send } from "lucide-react"

interface IRActionPanelProps {
    ir: any
    userRole: string
    currentRev: number
}

export function IRActionPanel({ ir, userRole, currentRev }: IRActionPanelProps) {
    const [loading, setLoading] = useState(false)
    const [action, setAction] = useState<'NONE' | 'APPROVE' | 'REJECT' | 'RESUBMIT'>('NONE')
    const router = useRouter()

    const canApprove = ['ADMIN', 'PM'].includes(userRole) && ir.status === 'PENDING'
    const canResubmit = ['ADMIN', 'PM', 'SITE_ENGINEER'].includes(userRole) && (ir.status === 'REJECTED' || ir.status === 'REVISE_RESUBMIT')
    ...
    async function handleStatusUpdate(newStatus: string, formData: FormData) {
        ...
        if (res.success) {
            toast.success(`IR ${newStatus === 'APPROVED' ? 'Approved' : 'Rejected'} Successfully`)
            setAction('NONE')
            router.refresh()
        } else {
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                    <div className="flex justify-between items-center">
                        <h3 className="font-bold text-emerald-800 flex items-center gap-2">
                            ...
                            اعتماد الفحص (Approval)
                        </h3>
                        <Button variant="ghost" size="sm" onClick={() => setAction('NONE')}>Cancel</Button>
                    </div>

                    <form action={(fd) => handleStatusUpdate('APPROVED', fd)} className="space-y-4">
                        <div className="grid md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <Label className="text-xs font-bold text-slate-600">إرفاق النسخة الموقعة (Signed Copy) *</Label>
                            ...
                            <div className="space-y-2">
                                <Label className="text-xs font-bold text-slate-600">ملاحظات (Comments)</Label>
                        ...
                        <div className="flex justify-end pt-2">
                            <Button type="submit" disabled={loading} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-8">
                                {loading ? "جاري المعالجة..." : "تأكيد الاعتماد (Confirm Approval)"}
                            </Button>
                        </div>
                    </form>
                </div>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <form action={(fd) => handleStatusUpdate('REJECTED', fd)} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input type="file" name="finalDocument" accept=".pdf,.png,.jpg" className="bg-slate-50" />
                        </div>
                        <div className="flex justify-end pt-2">
            ...
            {action === 'RESUBMIT' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <form action={handleResubmit} className="space-y-4">
                        <div className="space-y-2">
                            <Label className="text-xs font-bold text-slate-600">إرفاق الطلب المصحح (New Revision File) *</Label>
